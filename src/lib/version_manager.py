...
class VersionManager:
    ...
    def update_python_version(self, version: str) -> None:
        ...
        self._update_generators(version)
        ...
        for file_path in files:
            ...
            self._update_file_content(
                ...
                [
                    ...
                    (r'target_version\s*=\s*\[[^\]]*\]', f'target_version = ["{target_version}"]'),
                    (r'target-version\s*=\s*\[[^\]]*\]', f'target-version = ["{target_version}"]'),
                    (r'min_python_version:\s*"[^"]*"', f'min_python_version: "{version}"'),
                ]
            )
    ...
    def _update_generators(self, version: str) -> None:
        ...
        # This is more complex due to multiple version patterns in the file
        # We'll need to handle different contexts
        content = generators_path.read_text()
        
        # Update Python versions in GitHub Actions
        content = re.sub(
            r'python-version:\s*\[[^\]]*\]',
            f'python-version: ["3.9", "3.10", "3.11", "{version}"]',
            content
        )
        ...
        content = re.sub(
            r'node-version:\s*\[[^\]]*\]',
            f'node-version: ["18", "20", "{version}"]',
            content
        )
        
        # Update specific version references
        content = re.sub(
            r'python-version:\s*"[^"]*"',
        ...
        self.console.print("✅ Updated generators.py")
    
    def _update_file_content(self, file_path: Path, patterns: List[Tuple[str, str]]) -> None:
    ...
    def show_current_versions(self) -> None:
        ...
        categories = {
            ...
            "Tools": ["black", "isort", "flake8", "mypy", "pytest", "prettier", "eslint", "jest", "typescript"],
            "GitHub Actions": ["actions_checkout", "actions_setup_python", "actions_setup_node", "codeql_init", "codeql_analyze"]
        }
    ...
    def validate_versions(self) -> List[str]:
        ...
        if python_version and python_min:
            try:
                if tuple(map(int, python_version.split("."))) < tuple(map(int, python_min.split("."))):
                    errors.append(f"Python version {python_version} is below minimum {python_min}")
            except ValueError:
        ...
        if node_version and node_min:
            try:
                if int(node_version) < int(node_min):
                    errors.append(f"Node.js version {node_version} is below minimum {node_min}")
            except ValueError:
