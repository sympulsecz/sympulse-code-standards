...
class TestProjectConfigurator:
    ...
    def test_configure_project_with_language_and_name(
        self, mock_console, mock_int_prompt, mock_confirm, mock_prompt
    ):
        """Test configuring project when both language and name are provided."""
        # Setup mocks
        ...
        mock_confirm.ask.side_effect = [
            True,  # git_enabled
            True,  # git_branch_protection
            True,  # git_commit_template
            True,  # contributing_enabled
            True,  # issue_templates_enabled
            True,  # pr_templates_enabled
            True,  # code_of_conduct_enabled
            True,  # pre_commit_enabled
            True,  # testing_enabled
            True,  # coverage_enabled
        ]
        mock_int_prompt.ask.side_effect = [
            1,  # reviewers_count
            80,  # coverage_threshold
        ]

        configurator = ProjectConfigurator()
        config = configurator.configure_project("python", "test-project")

        # Verify basic info was collected
        ...
        assert config["contributing_enabled"] is True
        assert config["code_of_conduct_enabled"] is True
        assert config["issue_templates_enabled"] is True
        assert config["pr_templates_enabled"] is True
        assert config["git_commit_template"] is True

        # Verify contributing guidelines
        assert config["contributing"]["conventional_commits"] is True
        assert config["contributing"]["pr_required"] is True
        assert config["contributing"]["review_required"] is True
        assert config["contributing"]["reviewers_count"] == 1

        # Verify code quality tools
        assert config["code_quality"]["pre_commit_enabled"] is True
        assert config["code_quality"]["testing_enabled"] is True
        assert config["code_quality"]["coverage_enabled"] is True
        assert config["code_quality"]["coverage_threshold"] == 80

    @patch("src.cli.prompts.Prompt")
    @patch("src.cli.prompts.Confirm")
    @patch("src.cli.prompts.IntPrompt")
    @patch("src.cli.prompts.console")
    def test_configure_project_without_language(
        self, mock_console, mock_int_prompt, mock_confirm, mock_prompt
    ):
        """Test configuring project when language is not provided."""
        # Setup mocks for language selection
        mock_prompt.ask.side_effect = [
        ...
        configurator = ProjectConfigurator()
        config = configurator.configure_project(None, "test-project")
    ...
    @patch("src.cli.prompts.IntPrompt")
    @patch("src.cli.prompts.console")
    def test_configure_project_without_name(
        self, mock_console, mock_int_prompt, mock_confirm, mock_prompt
    ):
        """Test configuring project when name is not provided."""
        # Setup mocks for name and language selection
        mock_prompt.ask.side_effect = [
            "my-awesome-project",  # name selection
        ...
        configurator = ProjectConfigurator()
        config = configurator.configure_project("python", None)
    ...
    @patch("src.cli.prompts.IntPrompt")
    @patch("src.cli.prompts.console")
    def test_configure_project_without_language_and_name(
        self, mock_console, mock_int_prompt, mock_confirm, mock_prompt
    ):
        """Test configuring project when neither language nor name is provided."""
        # Setup mocks for name and language selection
        mock_prompt.ask.side_effect = [
            "my-awesome-project",  # name selection
            "typescript",  # language selection
        ...
        configurator = ProjectConfigurator()
        config = configurator.configure_project(None, None)
    ...
    @patch("src.cli.prompts.Confirm")
    @patch("src.cli.prompts.console")
    def test_configure_typescript_tools(self, mock_console, mock_confirm, mock_prompt):
        """Test TypeScript-specific tool configuration."""
        configurator = ProjectConfigurator()
        ...
        config = configurator._configure_code_quality_tools("typescript")
        ...
        assert tools["bundler"] == "vite"

    @patch("src.cli.prompts.Prompt")
    @patch("src.cli.prompts.Confirm")
    @patch("src.cli.prompts.console")
    def test_configure_go_tools(self, mock_console, mock_confirm, mock_prompt):
        """Test Go-specific tool configuration."""
        configurator = ProjectConfigurator()
        ...
        config = configurator._configure_code_quality_tools("go")

        # The method returns a dict with "code_quality" key
        assert "code_quality" in config
        tools = config["code_quality"]

        # Go tools are configured differently - check that the method doesn't crash
        assert "pre_commit_enabled" in tools
