...
class PythonValidator(BaseValidator):
    ...
    def _check_naming_conventions(self, tree: ast.AST, file_path: Path) -> List[ValidationIssue]:
                ...
                if isinstance(node, ast.Assign):
                    for target in node.targets:
                        if isinstance(target, ast.Name):
                            ...
                            if target.id.isupper() and not re.match(constant_pattern, target.id):
                                issues.append(ValidationIssue(
                                    severity="warning",
    ...
    def _check_import_order(self, tree: ast.AST, file_path: Path, 
                           expected_order: List[str]) -> List[ValidationIssue]:
    ...
    def _check_unused_imports(self, tree: ast.AST, file_path: Path) -> List[ValidationIssue]:
    ...
    def _check_code_structure(self, tree: ast.AST, file_path: Path) -> List[ValidationIssue]:
    ...
    def _check_config_files(self, project_path: Path) -> List[ValidationIssue]:
        ...
        requirements_files = ["requirements.txt", "requirements-dev.txt", "Pipfile", "poetry.lock"]
    ...
    def _check_pyproject_toml(self, file_path: Path, config_rules: Dict[str, Any]) -> List[ValidationIssue]:
        ...
        try:
            ...
            if "project" in config:
                ...
                if requires_python:
                    min_version = config_rules.get("min_python_version", "3.8")
    ...
    def _check_requirements_file(self, file_path: Path, config_rules: Dict[str, Any]) -> List[ValidationIssue]:
...
class TypeScriptValidator(BaseValidator):
    ...
    def _check_naming_conventions(self, content: str, file_path: Path) -> List[ValidationIssue]:
    ...
    def _check_import_order(self, content: str, file_path: Path, 
                           expected_order: List[str]) -> List[ValidationIssue]:
        """Check that imports follow the expected order."""
        issues = []
    ...
    def _check_code_structure(self, content: str, file_path: Path) -> List[ValidationIssue]:
        """Check code structure and organization."""
        issues = []
        structure_rules = self.rules.get("code_structure", {})
    ...
    def _check_package_json(self, file_path: Path, config_rules: Dict[str, Any]) -> List[ValidationIssue]:
    ...
    def _check_tsconfig_json(self, file_path: Path, config_rules: Dict[str, Any]) -> List[ValidationIssue]:
